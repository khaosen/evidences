...
import styles from "../../../../css/InterceptionChooser.module.css";
...
export default function InterceptionChooser(props: InterceptionChooserProps) {
    ...
    return <div style={{ width: "50%", height: "100%", padding: "20px", display: "flex", flexDirection: "column", gap: "10px", background: "rgba(255, 255, 255, 0.2)", boxShadow: "0 4px 30px rgba(0, 0, 0, 0.1)", border: "2px solid rgba(255, 255, 255, 0.8)", borderRadius: "16px" }}>
        ...
        <div ref={scrollRef} className={styles.interception__chooser} onScroll={handleScroll}>
            <div style={{ width: "100%", display: "flex", flexDirection: "column", flexGrow: 1, alignItems: "start" }}>
                {interceptions.length == 0
                    ? <div style={{ height: "100%", width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px" }}>
                        ...
                        <p style={{ fontSize: "20px", lineHeight: "20px", margin: 0 }}>{t("laptop.desktop_screen.wiretap_app.latest_actions.no_actions_available")}</p>
                    </div>
                    : interceptions.map((interception) => 
                        <div key={interception.id} className={`${styles.interception} hoverable`} onClick={() => openProtocol(interception)}>
                            ...
                            <div style={{ width: "calc(100% - 50px)", display: "flex", flexDirection: "column" }}>
                                <p style={{ fontSize: "20px" }}>{t("laptop.desktop_screen.wiretap_app.latest_actions.action_duration", formatDate(interception.startedAt), formatTime(interception.startedAt), formatTime(interception.endedAt))}</p>
                                <p style={{ fontSize: "30px" }}>{t(`laptop.desktop_screen.wiretap_app.latest_actions.actions.${interception.type}`, interception.observer, interception.target)}</p>
                            </div>
                        </div>
                    )
                }
